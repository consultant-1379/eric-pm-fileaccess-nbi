...
pipeline {
    ...
    stages {
        ...
        stage('Checkout Cloud-Native SG Git Repository') {
            steps {
                git branch: 'master',
                    url: '${GERRIT_MIRROR}/OSS/ENM-Parent/SQ-Gate/com.ericsson.oss.containerisation/eric-pm-fileaccess-nbi'
			   sh '''
                    git remote set-url origin --push https://${GERRIT_HTTP_CREDENTIALS_FUser}@${GERRIT_CENTRAL_HTTP_E2E}/OSS/ENM-Parent/SQ-Gate/com.ericsson.oss.containerisation/eric-pm-fileaccess-nbi
               '''
            }
        }
        stage('Helm Dep Up ') {
        ...
        stage('Merge values files') {
            steps{
                 script {
                     appconfig_values = sh (script: "ls ${WORKSPACE}/chart/eric-pm-fileaccess-nbi/appconfig/ | grep values.yaml", returnStatus: true)
                     ...
                     sh '''
                         if git status | grep 'values.yaml' > /dev/null; then
                            git add chart/eric-pm-fileaccess-nbi/values.yaml
        ...
        stage('ADP Image Design Rule Check') {
            steps {
                sh "${bob} test-image"
                archiveArtifacts '.bob/main_image/SG-image-design-rule-check-report.html'
            }
            post {
                failure {
                    script {
    ...
    post {
        ...
        always {
            script {
                sh "rm -rf build/grype-reports"
                sh "rm -rf build/trivy-reports"
            }
        }
    }
}
